...
def launch (log_name="", run_time=3, server_isn=None):
  def setup ():
    ...
    # there'll be 10 packets w payload
    sapp = s1._new_resocket_app(ClosingRXApp, port=1000, listen=True)
    capp = c1.new_fast_sender(data=13800, ip=s1.netdev.ip_addr, port=1000, delay=0.5)
    ...
    def do_score ():
      ...
      tester.expect_eq(10, payload_pkts, "10 payload packets, no retransmissions")

    class drop_acks(object):
      def __init__ (self):
        self.num_acks = 0

      def __call__ (self, dev, packet):
        tcp = packet.find("tcp")
        if not tcp or tcp.SYN or tcp.FIN:
          return False
    ...
    topo.get_wire(r1,c1).drop_conditions.append(drop_acks())
