...
def launch (log_name="s2_rcvpkts", data="", drop_count=0,
  ...
  def setup ():
    ...
    def do_score ():
      ...
      tester.expect_eq(3, num_pkts_payload, "3 packets with payload")

      # num acks = syn, 3 payload, maybe 2 from fin,
      num_server_acks = sum(1 for p in pkts if p._server and p.ACK)
      tester.expect_true(4 <= num_server_acks <= 6, "4 <= num_server_acks <= 6")

      tester.expect_true(11 <= len(pkts) <= 14, "11 <= num pkts <= 14")
