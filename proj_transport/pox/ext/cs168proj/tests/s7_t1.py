...
def launch (log_name="", run_time=3, server_isn=None):
  def setup ():
    ...
    # sapp calls close() after 1 sec
    # capp calls close() immediately after connect()
    sapp = s1._new_resocket_app(DelayedCloseApp, port=1000, listen=True)
    capp = c1.new_basic_state_trans(ip=s1.netdev.ip_addr, port=1000, delay=0.1)
    ...
    def do_score ():
      ...
      # FIN+ACK
      tester.expect_true(from_client(pkts[3]), "4th pkt comes from client")
      tester.expect_true(pkts[3].ACK and pkts[3].FIN, "4th pkt is FIN+ACK")

      # ACK
      tester.expect_true(from_server(pkts[4]), "5th pkt comes from server")
      tester.expect_eq(FIN_WAIT_1, pkts[4]._client_state, "state is LAST_ACK when r2 receives ACK")
      tester.expect_true(pkts[4].ACK, "5th pkt is ACK")

      # FIN+ACK
      tester.expect_true(from_server(pkts[5]), "6th pkt comes from server")
      tester.expect_eq(FIN_WAIT_2, pkts[5]._client_state, "state is LAST_ACK when r2 receives ACK")
      tester.expect_true(pkts[5].ACK and pkts[5].FIN, "6th pkt is FIN+ACK")

      # ACK
      tester.expect_true(from_client(pkts[6]), "7th pkt comes from client")
