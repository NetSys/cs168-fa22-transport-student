...
def launch (log_name="", data="", drop_count=0,
  ...
  def setup ():
    ...
    capp = c1.new_fast_sender(data=1990, ip=s1_ip, port=1000, delay=0.5)
    ...
    def do_score ():
      tester.expect_eq(("*" * 1990).encode('ascii'), sapp.all_rx[0][1], "payload correctly sent")

      pkts_payload = [p for p in pkts if p.payload]
      tester.expect_eq(10, len(pkts_payload), "10 packets with payload")
